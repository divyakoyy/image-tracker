%   function [g, skernel, dkernel] = gradn(img, sigma, trunc)
%
% g{i} is the i-th component of the gradient of img. The input image can
% have any number of dimensions.
%
% The factor sigma is the standard deviation of the Gaussian function used
% for differentiation; if left unspecified, it defaults to 1 pixel.
%
...
%
% [TODO: write a faster C version that doesn't require all this data
% reshuffling]

    function out = conv1(in, kernel, dim)
        if ~isvector(kernel)
        ...
        outflat = imfilter(inflat, kernel(end:-1:1), 'same', 'replicate');
